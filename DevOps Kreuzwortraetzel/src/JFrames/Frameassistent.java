...
public class Frameassistent {
	public Frameassistent(JFrame jf, JPanel contentPane, JTextPane textPane, JButton hauptmenu, Font buttonfont,
			JTextField fragefeld, JLabel hintergrundBild, JLabel PunkteLabel, JScrollPane scrollPane, JTextField[][] tf,
			int width, int height, JButton testButton, String[][] aw, JLabel motivationgif) {
	...
	public void initJFrameStandards(JFrame jf, JPanel contentPane) {
		...
		jf.setIconImage(Toolkit.getDefaultToolkit()
				.getImage(Mittel.class.getResource("/JFrames/pics/Team STar Mario (1).png")));
	...
	void initFrageFeld(JTextField fragefeld) {
		...
		fragefeld.setBorder(
				new TitledBorder(null, "Fragefeld", TitledBorder.LEADING, TitledBorder.TOP, null, Color.green));
	}
	...
	void inittf(JTextField[][] tf, int width, int height, JPanel contentPane, JTextField fragefeld, JButton testButton,
			String[][] aw) {
		for (int i = 0; i < 21; i++) {
			for (int j = 0; j < 20; j++) {
				Font f = new Font("Comic Sans", Font.BOLD, 16);
	...
	public void seteinfachFocusListener(String[][] aw, JTextField[][] tf) {
		for (int i = 0; i < 21; i++) {
			for (int j = 0; j < 20; j++) {
				if (!(aw[i][j].equals(" "))) {
	...
	public void setmittelFocusListener(String[][] aw, JTextField[][] tf) {
		for (int i = 0; i < 21; i++) {
			for (int j = 0; j < 20; j++) {
				if (!(aw[i][j].equals(" "))) {
	...
	public void setschwerFocusListener(String[][] aw, JTextField[][] tf) {
		for (int i = 0; i < 21; i++) {
			for (int j = 0; j < 20; j++) {
				if (!(aw[i][j].equals(" "))) {
